...
namespace FreeHandGestureFramework
{
    public sealed class GestureHandler
    {
        ...
        ///<summary>Defines the (hardware) platform for the GestureHandler instance.
        ///This value must be set, otherwise GestureHandler.Instance will return null.</summary> 
        public Platforms Platform;
        ///<summary>The list of gestures GestureHandler will try to recognize while active</summary>
        public List<Gesture> Gestures;
        private static bool _platformSet = false;//The platform must be set before Instance returns this instance
        private bool _isActive;//If not active, no gesture recognition is done
        private DateTime _timeOfLastUpdate;//Time stamp of the last call of Update()
        ...
        ///<summary>Returns the only instance of GestureHandler. If the platform is not yet set by calling 
        ///SetPlatform(), null is returned.</summary>
        public static GestureHandler Instance 
        { 
            get 
            { 
                if(!_platformSet)return null;
        ...
        ///<summary>Sets the (hardware) platform for this instance of GestureHandler.
        ///This method has to be called once before Instance() will return the GestureHandler instance.</summary> 
        ...
        public static void SetPlatform(Platforms platform, HandSkeletonBase customHandSkeleton = null)
        {
            ...
            {
                throw new System.ArgumentException("When initializing GestureHandler to a custom platform, customHandSkeletons must not be null. Please use a custom class derived from FreeHandGestureFramework.HandSkeletonBase.");
            }

            if (_platformSet == true) //In case SetPlatform() was called in order to CHANGE the platform
            {
                Instance.Initialize();
            }
            _platformSet = true;
        ...
        {
            if (!_isActive || Gestures.Count == 0 || currentHands==null) return;//no gesture recognition if inactive or no hands are provided
            ...
            else //A gesture is active, check for next gesture stage, gesture release, or dwell time over.
            {
                ...
                else if (activeStage.DwellTime > 0 && (activeGesture.GetMillisSinceStageRecognized(_timeOfLastUpdate) < activeStage.DwellTime))
                {
                ...
                else
                {
                               ...
                               >=activeGesture.Stages[activeStage.GetNextStageIndex(TransitionCondition.NextPoseRecognized)].RecognitionConfidence)
                    {
                        ...
                        targetStage=activeStage.GetNextStageIndex(TransitionCondition.NextPoseRecognized);
                            ...
                            && activeGesture.GetMillisSinceStageRecognized(_timeOfLastUpdate) > activeStage.DwellTime + activeStage.ReleaseWaitingTime
        ...
        private void Initialize()
        {
            ...
            _isActive = false;
            _timeOfLastUpdate = DateTime.MinValue;
            ActiveGesture = -1;
        }
        ...
        {
            //Invoke events of GestureHandler class
            switch (type)
            {
        ...
        private class Nested
        {
            ...
            internal static readonly GestureHandler instance = new GestureHandler();
        }  
